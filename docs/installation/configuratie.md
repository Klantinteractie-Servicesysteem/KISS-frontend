...
| `ELASTIC_BULK_HTTP_TIMEOUT_MINUTES`  | Timeout in minuten voor de bulk http requests richting elasticsearch ten behoeve van het synchroniseren van gegevens naar de zoekindex. <details><summary>Meer informatie</summary> Default waarde is `10`. Pas deze waarde aan als je grote hoeveelheden informatie synchroniseert en timeouts ondervindt in de sync jobs.  </details>              |
