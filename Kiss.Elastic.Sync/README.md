...
1. Make sure all secrets from the KISS readthedocs are present:
   https://kiss-klantinteractie-servicesysteem.readthedocs.io/nl/stable/installation/configuratie.html#kiss-elastic-sync
2. Set up a port forward for your Enterprise Search environment on port 3002, ex: `kubectl port-forward your-service 3002`
3. Set up a port forward for your Enterprise Search environment on port 9200, e.g.: `kubectl port-forward your-service 9200`
4. Build the tool using docker-compose: `docker compose build`
5. Run the tool using docker-compose: `docker compose run kiss.elastic.sync [ARGS...]`
