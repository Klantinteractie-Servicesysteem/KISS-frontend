...
try
{
    ...
    builder.Services.AddHttpClient("elasticsearch-metadata", httpClient => { httpClient.BaseAddress = new Uri(builder.Configuration["ELASTIC_BASE_URL"]); httpClient.SetBasicAuthentication(builder.Configuration["ELASTIC_USERNAME"], builder.Configuration["ELASTIC_PASSWORD"]); }).ConfigurePrimaryHttpMessageHandler(() => new HttpClientHandler { ServerCertificateCustomValidationCallback = HttpClientHandler.DangerousAcceptAnyServerCertificateValidator });
    builder.Services.AddSingleton<ElasticsearchMetadataCache>(sp => new ElasticsearchMetadataCache(sp.GetRequiredService<IHttpClientFactory>().CreateClient("elasticsearch-metadata"), sp.GetRequiredService<ILogger<ElasticsearchMetadataCache>>()));
