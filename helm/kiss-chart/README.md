...
| settings.syncJobs.website | list | `[]` | Optional fields: `schedule` (cron, default: `*/59 * * * *`), `historyLimit`, `outputIndex`, `maxCrawlDepth`, `maxExtractedLinksCount`, `maxUniqueUrlCount`, `purgeCrawlEnabled`, `fullHtmlExtractionEnabled`, `excludeTags` (default: `[header, footer, nav]`), `userAgent`, `threadsPerCrawl`, `crawlRules`. e.g. crawlRules to block login pages: crawlRules:   - policy: deny     type: contains     pattern: /login |
| tolerations | list | `[]` |  |
